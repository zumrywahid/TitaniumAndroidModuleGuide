	...
	implements KrollProxyListener 
{
	...
	private KrollFunction myownCallBack=null;
	...
	public void registerCallbacks(HashMap args)
	{
		...
		if(args.containsKey("myown")){
			
			callback=args.get("myown");
			if(callback instanceof KrollFunction){
				myownCallBack=(KrollFunction)callback;
			}
	...
	@Kroll.method
	public void MyOwnMethodCallBack(){
		
		Log.w(LCAT,"[KROLLDEMO] MyOwnMethodCallBack called");
		
		// The 'callSync' method of the KrollCallback object can be used to directly 
				// call the associated JavaScript function and get a return value.
				HashMap<String, String> event = new HashMap<String, String>();
				event.put("email", "test@gmail.com");
				event.put("website", "www.test.info");
				
			    myownCallBack.call(getKrollObject(), event);
